...
| `INT_DIV` | `1B` | `rd: Reg`, `ra: Reg`, `rb: Reg` | `regs[rd] = int(a / b)`, truncating toward zero; `a / 0 = 0`; `IntOverflow` for `INT_MIN / -1` |
| `INT_MOD` | `1C` | `rd: Reg`, `ra: Reg`, `rb: Reg` | `regs[rd] = int(a rem b)`, sign of `a`; `a mod 0 = a` |
| `INT_SUB_SAT` | `1D` | `rd: Reg`, `ra: Reg`, `rb: Reg` | `regs[rd] = int(a - b)` if `a > b`, else `int(0)`; `IntOverflow` if out of 24-bit range |
...
| `INT_SHL` | `27` | `rd: Reg`, `ra: Reg`, `rb: Reg` | `regs[rd] = int(a * 2^b)`; `IntOverflow` if out of 24-bit range (so any `b >= 24` with `a != 0`); `b < 0` gives `0` |
